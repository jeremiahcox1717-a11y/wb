...
import { isStudioConfigured, safeNextPath, SESSION_COOKIE, verifySessionToken } from "@/lib/auth";
...
const errors: Record<string, string> = {
  ...
  config: "The site is locked until ADMIN_PASSWORD is set (8+ characters) on the server.",
};
    ...
    <main className="flex min-h-screen items-center justify-center bg-[#111114] px-6 text-[#f4ece3]">
      <form action={loginAction} className="w-full max-w-sm">
        ...
        <p className="mt-3 text-sm leading-6 text-[#b9a89a]">
          This whole website is locked. Nobody else can see the pages, the studio, or the data. Use the owner
          password set on the server.
        </p>
        {!configured ? (
          <p className="mt-8 border border-[#3a3128] bg-[#1e1914] p-4 text-sm leading-6">
            The site is locked until <code className="text-[#d4a574]">ADMIN_PASSWORD</code> is set (8+ characters) on
