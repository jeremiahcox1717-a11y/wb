import { isPlaceholderBrand } from "../client-site";
...
function keepIdentity(previous: Site, nextName?: string) {
  const placeholder = isPlaceholderBrand(previous.identity.siteName);
  return {
    ...previous.identity,
    siteName: nextName || previous.identity.siteName,
    ownerName: placeholder ? nextName || previous.identity.ownerName : previous.identity.ownerName,
  };
}

function brandName(previous: Site, fallback: string) {
  return isPlaceholderBrand(previous.identity.siteName) ? fallback : previous.identity.siteName;
}

export const templates = {
  bakery(previous: Site): Site {
    const name = brandName(previous, "Hearth & Crumb");
    return stamp({
      identity: {
  ...
  coffee(previous: Site): Site {
    const name = brandName(previous, "Northroom Coffee");
    return stamp({
      identity: {
        ...keepIdentity(previous, name),
        siteName: name,
  ...
  restaurant(previous: Site): Site {
    const name = brandName(previous, "Supper House");
    return stamp({
      identity: {
        ...keepIdentity(previous, name),
        siteName: name,
  ...
  fitness(previous: Site): Site {
    const name = brandName(previous, "Ironroom");
    return stamp({
      identity: {
        ...keepIdentity(previous, name),
        siteName: name,
  ...
  agency(previous: Site): Site {
    const name = brandName(previous, "Field Office");
    return stamp({
      identity: {
        ...keepIdentity(previous, name),
        siteName: name,
...
export function genericBusiness(previous: Site, kind: string): Site {
  ...
  const name = brandName(previous, titled);
  return stamp({
    identity: {
      ...keepIdentity(previous, name),
      siteName: name,
