...
export function answerLocally(site: Site, message: string): string {
  ...
  if (/^(hi|hello|hey|yo|sup)\b/i.test(text)) {
    return `Hi ${name.split(" ")[0]}. Ask me anything — the time, a question, or a new website for someone else. This homepage stays yours.`;
  }
  ...
  if (/\b(what can you do|help|how (?:do i|does this) work|what is this)\b/i.test(lower)) {
    return [
      "This private page is Jordan Bennett’s and it stays that way.",
      "Ask me to build a NEW website for someone else — “build a white and blue site”, a bakery, or paste a public https link to clone. Each one gets its own public link under /s/…",
      "On this desk: URL maker (search then buy a real .com), URL scanner, name scanner, currency converter, and a notebook.",
    ].join(" ");
  }
  ...
  if (/\b(how (?:do i|can i) (?:change|edit|rebuild|make))\b/i.test(lower) || /\bbuild(?:er)? bot\b/i.test(lower)) {
    return "Tell me what to build for someone else — clone a public https link, a white and blue page from scratch, a bakery — and I will make a new site at /s/… This Jordan Bennett homepage stays.";
  }
