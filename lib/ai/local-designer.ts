import { blankClientSite } from "../client-site";
...
export function wantsPersonalHome(text: string) {
  const t = text.trim();
  if (!t) return false;
...
function personalFresh(previous: Site): Site {
  const next = blankClientSite();
  next.identity = {
    ...next.identity,
    ownerName: previous.identity.ownerName || next.identity.ownerName,
    ...
    siteName: previous.identity.siteName || previous.identity.ownerName || next.identity.siteName,
  };
  if (previous.identity.ownerName && previous.identity.ownerName !== "New site") {
    applyName(next, previous.identity.ownerName);
  }
  return next;
}

export function wantsFreshBuild(text: string) {
...
function sectionBuilders(): Record<string, SiteSection> {
  return {
    ...
    faq: {
      ...
      heading: "Questions",
      items: [
        { question: "Who is this website for?", answer: "This is a public site for a client. Anyone with the link can open it. The owner’s private homepage stays separate." },
        { question: "How fast do changes go live?", answer: "Immediately. The client site updates as soon as the bot replies." },
      ],
    },
    pricing: {
      type: "pricing",
