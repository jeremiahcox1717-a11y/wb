  ...
  const [turns, setTurns] = useState<Turn[]>([
    {
      role: "assistant",
      content:
        "Ask me to build a NEW website for someone else. Your Jordan Bennett homepage stays. Paste a https link to clone, or say “build a white and blue site”.",
    },
  ]);
  ...
  async function send(message: string) {
    ...
    try {
      ...
      const data = (await response.json()) as {
        ...
        warning?: string;
        changed?: boolean;
        project?: { slug: string; name: string; url: string; site?: Site } | null;
      };
      if (!response.ok) {
        setTurns((current) => [...current, { role: "assistant", content: data.error || "That did not work." }]);
        return;
      }
      if (data.project?.site) setSite(data.project.site);
      ...
      <aside className="flex w-full max-w-[28rem] shrink-0 flex-col border-r border-[#2a2a32]">
        <header className="flex items-center justify-between px-5 py-4">
          <div>
            ...
            <p className="mt-1 text-sm text-[#b9a89a]">New websites for other people. This homepage stays.</p>
          </div>
        ...
        <form onSubmit={onSubmit} className="border-t border-[#2a2a32] p-4">
          <textarea
            ...
            rows={3}
            placeholder="Build a bakery for someone else, or paste a https link to clone…"
