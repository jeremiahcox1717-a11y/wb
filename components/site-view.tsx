...
export function SiteView({
  ...
  const sections = publicSite
    ? page.sections.filter(
        (section) =>
          section.type !== "urlMaker" &&
          section.type !== "urlScanner" &&
          section.type !== "nameScanner" &&
          section.type !== "currencyCalculator" &&
          section.type !== "notebook",
      )
    : page.sections;
    ...
    <div className={preview ? "min-h-full" : "min-h-screen"} style={themeVars(site)}>
      ...
      <header className="mx-auto flex w-full max-w-6xl items-center justify-between px-6 py-6 md:px-10">
        {preview ? (
          ...
          <Link href={homeHref} className="text-lg tracking-tight no-underline" style={{ color: "var(--wb-text)" }}>
            <Display>{site.identity.siteName}</Display>
          </Link>
        )}
        <nav className="hidden gap-6 text-sm md:flex" style={{ color: "var(--wb-muted)" }}>
          {site.nav
            .filter((item) => !publicSite || !item.href.startsWith("#url") && item.href !== "#currency" && item.href !== "#notebook" && item.href !== "#name-scan")
            .map((item) =>
            preview ? (
      ...
      <main>{sections.map((section) => renderSection(section, site))}</main>
    </div>
  );
}
