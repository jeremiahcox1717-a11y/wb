...
export function SiteBuilderBot({ onProject }: { onProject: (project: ProjectCard) => void }) {
  ...
  const [turns, setTurns] = useState<Turn[]>([
    {
      role: "assistant",
      content:
        "I build NEW websites for other people. Your Jordan Bennett homepage stays as it is. Paste a public https link to clone, or say “build a white and blue site for a bakery”.",
    },
  ]);
    ...
    <div id="builder-bot" className="flex min-h-0 flex-1 flex-col">
      <header className="flex items-center gap-2 border-b border-[#2a2a32] px-3 py-2">
        ...
        <div>
          ...
          <p className="text-xs text-[#b9a89a]">{busy ? "Working…" : "New sites for other people — this homepage stays"}</p>
        </div>
      </header>
      ...
      <form onSubmit={onSubmit} className="flex items-end gap-2 border-t border-[#2a2a32] p-2">
        <label className="sr-only" htmlFor="builder-bot-input">
          Tell the bot a question or what website to build for someone else
        </label>
        <textarea
